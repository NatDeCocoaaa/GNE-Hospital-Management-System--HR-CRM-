...
function Sidebar({ role, collapsed, onToggle, activePath, onNavigate }) {
...
function Topbar({ role, user, onMobileMenuToggle }) {
  const config = NAV_CONFIG[role] ?? NAV_CONFIG.Doctor;
  ...
  return (
    <header className="flex items-center justify-between h-16 px-6 bg-white border-b border-surface-200 shadow-topbar shrink-0">
      ...
      <div className="flex items-center gap-3">
        ...
        {/* User pill */}
        <button className="flex items-center gap-2.5 pl-1 pr-3 py-1.5 rounded-xl hover:bg-surface-100 transition-colors group">
          {/* Avatar */}
          <span className="w-7 h-7 rounded-lg bg-primary-600 flex items-center justify-center text-white text-xs font-bold shrink-0">
          ...
          <ChevronDown className="w-4 h-4 text-surface-400 hidden sm:block group-hover:text-surface-600 transition-colors" />
        </button>
      </div>
    </header>
  );
...
function MobileOverlay({ open, onClose, role, activePath, onNavigate }) {
  if (!open) return null;
  return (
